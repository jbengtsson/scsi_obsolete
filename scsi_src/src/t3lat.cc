...
bool Lattice_Read(const char *fi_)
{
    ...
    for (list<string>::iterator i = elems.begin(); i != elems.end(); ++i) {
        ...
        glps_read(*i, str);
        for (int j=0; j<str.length();++j) str[j]=tolower(str[j]); //str.replace(i,1,1,tolower(str[i]));
        for (pos=0; pos<elemsdefined.size();++pos) if (!str.compare(elemsdefined[pos])) break;
        if (pos == elemsdefined.size()) {
            string unknown=*i; //strdup(*i);
            for(int k=0; k<unknown.length();++k) unknown[k]=tolower(unknown[k]);
        ...
        double         t, t1, t2, gap, QL = 0.0, QK, QK2;
...
//  cout << "Number of lines: " << lines.size() << endl;
    ...
    for (list<string>::iterator i = lines.begin(); i != lines.end(); ++i) {
        beamline=*i;
        for (int k=0; k<beamline.length();k++) beamline[k]=tolower(beamline[k]);
