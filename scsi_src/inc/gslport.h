// Converts gsl matrix to double precision nr matrix and creates new pointer
// to pointer.
#define GSL2NRDM1(d, m, p, s) 	double* d[m->size1+1];	\
  {							\
    int port_i;						\
    for(port_i = s; port_i <= (int)m->size1; port_i++) {	\
      d[port_i+1] = m->data + port_i*m->size2 - 1;	\
    }							\
  }							\
  double** p = d;
	
// Converts gsl matrix to double precision nr matrix and assigns to already
// existing pointer to pointer.
