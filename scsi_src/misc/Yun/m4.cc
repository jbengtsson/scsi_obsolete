...
int main(int argc, char *argv[])
{
  ...
  if (true) {
    ...
    get_cod_rms_scl_new(2); //trim coils aren't reset when finished
  ...
  if (false) {
	...
	IBS_BM(Qb, globval.eps, eps);  // use IBS_BM instead of old IBS
      }
    }
    ...
    if (false) {       
      ...
      n_turns = 483; // track for one synchr.osc. -> 1/nu_s (M4 bare @ 1.8MV -> 446, 20130515)
                     //                                     (M5 bare @ 560 kV -> 419)
